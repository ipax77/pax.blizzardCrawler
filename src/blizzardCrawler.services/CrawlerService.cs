...
public partial class CrawlerService
{
    ...
    private readonly HttpClient euHttpClient;
    private readonly HttpClient naHttpClient;
                          ...
                          ILogger<CrawlerService> logger)
    {
        ...
        this.logger = logger;
        this.euHttpClient = httpClientFactory.CreateClient("blizzardEU");
        this.naHttpClient = httpClientFactory.CreateClient("blizzardUS");

        tokenBucketSeconds = new(MaxRequestsPerSecond, 1000, 1000);
        tokenBucketHour = new(MaxRequestsPerHour, 3600000, 10000);
        CrawlerThreads = (int)(importOptions.Value.CrawlerThreadsCount * 0.60);
        RetryThreads = importOptions.Value.CrawlerThreadsCount - CrawlerThreads;
    }
    ...
    private async Task Job(CancellationToken token = default)
    {
        ...
        _ = matchRepository.StoreMatches();

        while (retryChannel.Reader.Count > 1000)
        {
                                    ...
                                    CancellationToken token)
    {
        ...
        (var matches, int statusCode, var newEtag) 
            = await GetMatchHistory(player, etag, dsOnly: true, token);
    ...
    public async Task GetPlayerMatchInfos(PlayerIndex player)
    {
        (var matches, int statusCode, var etag) = await GetMatchHistory(player, null);
    }
}
